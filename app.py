        ...
        if st.button("✨ 유사 문제 2개 초고속 생성 (기본1 + 응용1)", type="primary"):
            with st.spinner("Gemini가 단원 범위, 표, 그래프 조건에 맞춰 문제를 출제하고 있습니다..."):
                try:
                    ...
                    # ★ 꺾은선그래프/막대그래프 세로+가로 격자선(모눈종이) 출제 지침 강화
                    prompt = f"""
                    ...
                    2. **꺾은선그래프 / 막대그래프 / 히스토그램 격자선 규칙 (매우 중요):**
                       - 원본 문제에 **꺾은선그래프, 막대그래프, 히스토그램, 도수분포다각형**이 포함된 경우, **반드시 문제 본문에 실제 교과서와 똑같은 인라인 SVG 그래프(`<svg width="260" height="160" viewBox="0 0 260 160">...</svg>`)를 직접 작성하여 넣어라.**
                       - SVG 구성 필수 요소 (모눈종이 격자 완성):
                         * **가로 격자선뿐만 아니라, 각 점수/계급(예: 60점, 70점, 80점, 90점, 100점 / 1, 2, 3, 4, 5) 위치마다 위아래로 이어지는 세로 격자선(`<line x1="x" y1="y_top" x2="x" y2="y_bottom" stroke="#e0e0e0" stroke-width="1"/>`)을 빠짐없이 그려서 완전한 바둑판 모눈종이 격자망을 완성할 것.**
                         * 가로축 및 세로축 기준선: `stroke="#111111" stroke-width="1.5"`
                         * 축 단위 레이블: (점), (명), (권) 등
                         * 눈금 숫자 레이블: 가로축(60, 70, 80...) 및 세로축(0, 1, 2, 3, 4, 5, 6, 7, 8...)
                         * 꺾은선의 각 데이터 점(`<circle cx="..." cy="..." r="3.5" fill="..."/>`)은 **가로선과 세로선의 교차점에 정확히 찍히도록 좌표를 정렬**할 것.
                    3. **토너먼트 / 대진표 문제:**
                    ...
                    7. **문제 구성:**
                       - 1번 문제: 조건과 숫자만 바꾼 기본 다지기 문제 (모눈종이 격자선이 완비된 SVG 그래프 포함)
                       - 2번 문제: 같은 단원 개념 내에서 묻는 방식을 변형한 실력 키우기 문제 (모눈종이 격자선이 완비된 SVG 그래프 포함)
