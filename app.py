...
def format_math(text):
    ...
    # 10. 그래프/도형 SVG 다이어그램 흰색 카드 박스 감싸기
        ...
        if st.button("✨ 유사 문제 2개 초고속 생성 (기본1 + 응용1)", type="primary"):
            with st.spinner("Gemini가 단원 범위, 표, 그래프 조건에 맞춰 문제를 출제하고 있습니다..."):
                try:
                    ...
                    # ★ 막대그래프/히스토그램 세로+가로 격자선(모눈종이) + 다크모드 대응 글자색 강화
                    prompt = f"""
                    ...
                    2. **꺾은선그래프 / 막대그래프 / 히스토그램 격자선 규칙 (매우 중요):**
                       - 원본 문제에 그래프가 포함된 경우, **반드시 문제 본문에 실제 교과서와 똑같은 인라인 SVG 그래프(`<svg width="260" height="160" viewBox="0 0 260 160">...</svg>`)를 직접 작성하여 넣어라.**
                       - SVG 구성 필수 요소:
                         * 연한 회색 눈금 격자선 (`stroke="#e0e0e0" stroke-width="1"`)
                         * **각 점수/계급 위치마다 위아래로 이어지는 세로 격자선(`<line x1="x" y1="y_top" x2="x" y2="y_bottom" stroke="#e0e0e0" stroke-width="1"/>`)을 빠짐없이 그려서 바둑판 모눈종이 격자망을 완성할 것.**
                         * 가로축 및 세로축 기준선: `stroke="#111111" stroke-width="1.5"`
                         * **모든 텍스트 레이블(숫자, 축 이름):** 반드시 `fill="#000000"` (검정색)으로 설정하여 다크 모드 배경에서 선명하게 보이도록 할 것.
                         * 꺾은선 점(circle): `fill="#111111" stroke="#111111"`
                         * 막대 사각형: `fill="#ffe082" stroke="#333333"`
                    3. **토너먼트 / 대진표 문제:**
                       - 대진표인 경우 SVG로 그려라. (글자 색상 `fill="#000000"`)
                    4. **표(Table) 문제 작성 규칙:**
                       - 표가 필요한 경우 **마크다운 표 형식(`| 항목1 | 항목2 | ... |`)**으로 작성하고 셀 내부에 $를 쓰지 마라.
                    5. **문제 구성:**
                       - 1번/2번 문제 모두 원본과 동일한 구조의 그래프/표를 포함할 것.
