...
# ==========================================
# ★ 수식 렌더링, 표(Table), 막대그래프/도형 SVG 통합 엔진
# ==========================================
def _clean_cell(col):
...
def format_math(text):
    ...
    # 10. 막대그래프/토너먼트/도형 SVG 다이어그램 흰색 카드 박스 감싸기
        ...
        if st.button("✨ 유사 문제 2개 초고속 생성 (기본1 + 응용1)", type="primary"):
            with st.spinner("Gemini가 단원 범위, 표, 그래프 조건에 맞춰 문제를 출제하고 있습니다..."):
                try:
                    ...
                    # ★ 막대그래프/히스토그램 SVG, 토너먼트 SVG, 표, 빈칸 박스 출제 지침
                    prompt = f"""
                    ...
                    1. **단원 범위 준수 (선행 개념 절대 금지):**
                       - 원본 문제가 미분/도함수 단원이면 적분 기호($\\int$)를 쓰지 마라.
                       - 원본 문제가 통계/대푯값(평균, 중앙값, 최빈값) 단원이면 해당 개념 내에서만 변형하라.
                    2. **막대그래프 / 히스토그램 / 꺾은선그래프 문제 (매우 중요):**
                       - 원본 문제에 **막대그래프, 히스토그램, 도수분포다각형, 꺾은선그래프**가 포함된 경우, **반드시 문제 본문에 실제 교과서와 똑같은 인라인 SVG 그래프(`<svg width="250" height="150" viewBox="0 0 250 150">...</svg>`)를 직접 그려서 넣어라.**
                       - SVG 구성 요소 필수 포함:
                         * 연한 회색 눈금 격자선 (`stroke="#e0e0e0" stroke-width="1"`)
                         * 가로축, 세로축 기준선 (`stroke="#111111" stroke-width="1.5"`)
                         * 가로/세로 축 단위 및 레이블 (예: `<text x="230" y="130" font-size="11">(점)</text>`, `<text x="15" y="15" font-size="11">(명)</text>`)
                         * 눈금 숫자 (가로축 1, 2, 3, 4, 5 / 세로축 0, 1, 2, 3, 4, 5)
                         * 도수에 맞는 막대 사각형 (`<rect x="..." y="..." width="22" height="..." fill="#ffe082" stroke="#333" stroke-width="1.5"/>`)
                    3. **토너먼트 / 대진표 문제:**
                       - '대진표', '토너먼트' 문제인 경우 반드시 인라인 SVG(`<svg width="240" height="120">...</svg>`)로 대진 트리를 그려서 넣어라.
                    4. **표(Table) 문제 작성 규칙:**
                       - 표(도수분포표, 확률분포표 등)가 필요한 경우 반드시 **마크다운 표 형식(`| 항목1 | 항목2 | ... |`)**으로 작성하고 셀 내부에 불필요한 $를 쓰지 마라.
                    5. **영역 색칠하기 다이어그램:**
                       - 영역 색칠 문제인 경우 SVG로 각 영역(A, B, C, D)을 그려 넣어라.
                    6. **빈칸 채우기 및 증명 문제:**
                       - 빈칸은 반드시 `$\\boxed{{\\text{{ (가) }}}}$`, `$\\boxed{{\\text{{ (나) }}}}$` 형태로 작성할 것.
                    7. **문제 구성:**
                       - 1번 문제: 조건과 숫자만 바꾼 기본 다지기 문제 (원본과 동일한 막대그래프 SVG 포함)
                       - 2번 문제: 같은 단원 개념 내에서 묻는 방식을 변형한 실력 키우기 문제 (변형된 막대그래프 SVG 또는 표 포함)

                    [수식 작성 규칙]
                    1. 모든 수식, 분수식, 대푯값 기호는 `$수식$` 기호로 감싸라. (단, `<svg>...</svg>` 및 표 `|...|`는 $ 없이 일반 텍스트로 작성)
